...
test.describe("CDN", () => {
  ...
  test("Check redirect from bare domain to www.gov.uk is working for HTTP", async ({ page }) => {
    const response = await page.request.get("http://gov.uk", { maxRedirects: 0 });
    expect(response.status()).toBe(301);
    expect(response.headers()["location"]).toBe("https://gov.uk/");
  });

  test("Check redirect from bare domain to www.gov.uk is working for HTTPS and has HSTS enabled", async ({ page }) => {
  ...
  test("Check www.gov.uk redirect from HTTP to HTTPS is working", async ({ page }) => {
    const response = await page.request.get("http://www.gov.uk", { maxRedirects: 0 });
    expect(response.status()).toBe(301);
    expect(response.headers()["location"]).toBe("https://www.gov.uk/");
  });

  test("Check redirect from service domain to GOV.UK has HSTS enabled", async ({ page }) => {
