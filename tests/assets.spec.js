...
test.describe("Assets", { tag: ["@app-asset-manager"] }, () => {
  test(
  ...
  test("Check an asset can be served", async ({ request }) => {
...
test.describe("Draft assets", { tag: ["@app-asset-manager"] }, () => {
  test.use({ storageState: { cookies: [], origins: [] }, baseURL: publishingAppUrl("draft-assets") });

  test("Check a draft assets can be served", async ({ page }) => {
